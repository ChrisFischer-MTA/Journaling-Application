...
from .forms import AskJournalForm, JournalForm
...
def mood_list_creation(entry: JournalEntry):
    ...
    if entry.mood is None:
        return [
            '<span class="badge badge-pill badge-light">Journal is yet to be processed by AI</span>'
        ]
    # We expect that entry.mood is a valid python array
    try:
        # Hacky way of doing this, but, this is the best I can think of
        print(
            json.loads(entry.mood.replace("'", '"'))
        )  # AI will generate the list with either ' or ", this fixes it
        moodl = json.loads(entry.mood.replace("'", '"'))
        for element in moodl:
            print(element)
            if mood_colors.get(element) is not None:
                return_list.append(
                    f'<span class="badge badge-pill {mood_colors.get(element)}">{element}</span>'
                )
    except:
        return [
            '<span class="badge badge-pill badge-danger">Mood list generated by AI is invalid</span>'
        ]
    print(return_list)
    return return_list
...
def journals(request):
    ...
    return render(
        request, "journalindex.html", {"journal_entries": journal_entries}
...
def journal_question(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            prompt = "You are an unbiased AI agent who has been given the task of answering a query from a user. The qeury from the user will require you to consider one or more journal entries, some of which will be relevant (important! not all journal entries may be relevant.) Your task is to answer the question of the user as best as you can while considering these journals. Where plausible, cite the journals in your answer if they are relevant.\n Format your response in markdown."
            question = f"\nNow, consider the question from the user:\n```{form.cleaned_data['question']}```\n"
            journal_preamble = "Now, consider the following journal entries.\n"
            ...
            response = requests.post(
                ...
                json={
                    ...
                    "stream": False,
                    "prompt": prompt + question + journal_preamble,
                },
            )
            return render(
                request,
...
def journal_create(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new.user = (
                request.user
            )  # This WILL error when not logged in, but, that's fine
            new.save()
...
def report_detail(request, journal_id):
    report_entry = get_object_or_404(Report, id=journal_id, user=request.user)
