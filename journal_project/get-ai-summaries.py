...
from journalmain.models import Blurb, JournalEntry, Report
...
for entry in journalEntries:
    entry_key = str(entry.date.isocalendar()[0:2]) + f'_USER_{str(entry.user.id)}'
    # If it's this CURRENT week, it's not due for processing.
    if entry_key == str(datetime.datetime.now().isocalendar()[0:2]) + f'_USER_{str(entry.user.id)}':
        continue
