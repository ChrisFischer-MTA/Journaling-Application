...
def process_entry(entry, tries):
    ...
    response = requests.post(
        "http://ollama-intel-gpu:11434/api/generate",
        json={
            "model": "deepseek-r1:14b",
            "stream": False,
            "prompt": mood_prompt + f"```\n{entry.content}\n```",
        },
    )
...
for entry in journalEntries:
    ...
    if (
        entry_key
        == str(datetime.datetime.now().isocalendar()[0:2]) + f"_USER_{entry.user.id!s}"
    ):
        continue
...
for key, journal_entry in to_process.items():
    ...
    report_obj = Report(
        user=journals[0].user,
        type="w",
        title=f"Week {last_journal_date!s} Journal Report",
        content=response.json()["response"].split("</think>")[1].strip(),
    )
    report_obj.save()
