...
if SIGNAL_NUMBER is None:
    with open("/etc/enviroment") as ifp:
...
response = requests.get(
    f"http://signal-api:8080/v1/receive/{SIGNAL_NUMBER}", headers=headers, data=data
)
for message in response.json():
    if (
        message.get("envelope") is not None
        and message["envelope"].get("dataMessage") is not None
        and message["envelope"]["dataMessage"].get("message")
    ) is not None:
